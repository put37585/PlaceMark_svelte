...
export class LeafletMap {
  ...
  addMarker(location, popupText = "", layerTitle = "default") {
    ...
    if (popupText) {
      var popup = L.popup({autoClose: false, closeOnClick: true, closeButton: false});
  ...
  removeMarker(name) {
    this.imap.eachLayer((layer) => {
      if (layer._popup) {
        if (layer._popup._content == name) {
          this.imap.removeLayer(layer);
          return;
        }
      }
    });
